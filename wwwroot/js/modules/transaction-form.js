// transaction-form.js - Módulo unificado para formularios de transacciones (ventas y cotizaciones)
(function (window, $) {
    'use strict';
    ...
    App.transactionForm = {
        ...
        init: function (options) {
            this.options = $.extend({
                tipo: 'venta', // 'venta' o 'cotizacion'
                formSelector: '#ventaForm',
                searchProductoUrl: '/Ventas/BuscarProducto',
                tableSelector: '#productosTable'
            }, options);
            ...
            if (this.options.tipo === 'venta') {
                this.setupCotizacion();
                this.setupCreditoPersonal();
            } else if (this.options.tipo === 'cotizacion') {
        ...
        setupProductSearch: function () {
            ...
            $('#buscarProducto').off('click').on('click', function (e) {
                ...
                $.ajax({
                    url: self.options.searchProductoUrl,
                    type: 'POST',
            ...
            $('#agregarProducto').off('click').on('click', function (e) {
                ...
                $(self.options.tableSelector + ' tbody tr').each(function (i) {
                ...
                if (existe) {
                    // Actualizar cantidad
                    const row = $(self.options.tableSelector + ' tbody tr').eq(index);
                    ...
                    row.find('.subtotal').text(App.format.currency(nuevoSubtotal));
                    row.find('input[name$=".PrecioTotal"]').val(nuevoSubtotal.toFixed(2));
                } else {
                    // Crear nueva fila con valores correctamente formateados
                    const rowCount = $(self.options.tableSelector + ' tbody tr').length;
                    const itemPrefix = self.options.tipo === 'venta' ? 'ProductosVenta' : 'ProductosPresupuesto';

                    const newRow = `
                     <tr data-index="${rowCount}">
                         <td>
                             <input type="hidden" name="${itemPrefix}[${rowCount}].ProductoID" value="${self.productoActual.id}" />
                             <input type="hidden" name="${itemPrefix}[${rowCount}].CodigoAlfa" value="${self.productoActual.codigoAlfa}" />
                             <input type="hidden" name="${itemPrefix}[${rowCount}].CodigoBarra" value="${self.productoActual.codigoBarra}" />
                             <input type="hidden" name="${itemPrefix}[${rowCount}].Marca" value="${self.productoActual.marca}" />
                             <input type="hidden" name="${itemPrefix}[${rowCount}].NombreProducto" value="${self.productoActual.nombre}" />
                             <input type="hidden" name="${itemPrefix}[${rowCount}].PrecioUnitario" value="${precio.toFixed(2)}" />
                             <input type="hidden" name="${itemPrefix}[${rowCount}].PrecioTotal" value="${subtotal.toFixed(2)}" />
                             <input type="hidden" name="${itemPrefix}[${rowCount}].PrecioLista" value="${(self.productoActual.precioLista).toFixed(2)}" />
                             ${self.productoActual.codigoAlfa || self.productoActual.codigoBarra || self.productoActual.id}
                         </td>
                         <td>${self.productoActual.nombre}</td>
                         <td><input type="number" name="${itemPrefix}[${rowCount}].Cantidad" value="${cantidad}" min="1" class="form-control form-control-sm bg-dark text-light cantidad" /></td>
                 ...
                 `;
                    ...
                    $(self.options.tableSelector + ' tbody').append(newRow);
                }
        ...
        // Configurar crédito personal (solo para ventas)
        ...
        // Configurar opciones de cotización (solo para ventas)
        ...
        // Funcionalidad para cotizaciones (solo para cotizaciones)
        ...
        updateTotals: function () {
            ...
            $(self.options.tableSelector + ' tbody tr').each(function () {
            ...
            }).appendTo(self.options.formSelector);
        },
        ...
        reindexRows: function () {
            ...
            $(self.options.tableSelector + ' tbody tr').each(function (index) {
    ...
    // Inicializador para venta
    App.ventaForm = {
        init: function () {
            App.transactionForm.init({
                tipo: 'venta',
                formSelector: '#ventaForm',
                searchProductoUrl: '/Ventas/BuscarProducto',
                tableSelector: '#productosTable'
    ...
    // Inicializador para cotización
    App.cotizacionForm = {
        init: function () {
            App.transactionForm.init({
                tipo: 'cotizacion',
                formSelector: '#cotizacionForm',
                searchProductoUrl: '/Cotizaciones/BuscarProducto',
                tableSelector: '#productosTable'
            });
        }
    };
