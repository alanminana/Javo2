...
const inventory = {
    ...
    initProductoForm() {
        ...
        const saveBtn = document.querySelector('#saveProducto');
        if (saveBtn) saveBtn.addEventListener('click', () => this.handleProductoSubmit());
    },

    initProductoCategorias() {
        loadDropdown('/Productos/GetRubros', {}, '#rubroSelect', { placeholder: 'Seleccione rubro...' });

        const rubroSelect = document.querySelector('#rubroSelect');
        if (rubroSelect) {
            rubroSelect.addEventListener('change', e => {
                loadDropdown('/Productos/GetSubRubros', { rubroID: e.target.value }, '#subrubroSelect',
                    { placeholder: 'Seleccione subrubro...' });
            });

            const initial = rubroSelect.value;
            if (initial) {
                loadDropdown('/Productos/GetSubRubros', { rubroID: initial }, '#subrubroSelect',
                    { placeholder: 'Seleccione subrubro...' });
            }
        }
    },
    ...
    initCatalogoFormDropdowns() {
        ...
        catSelect.addEventListener('change', e => {
            loadDropdown('/Catalogo/GetSubCategorias', { categoriaID: e.target.value }, '#subcategoriaSelect',
